...
# Use format with DC and MC only
  ...
  return (
    ...
    >
      ...
      {settings.globalDepositNote.trim() ? (
        <motion.div className="mb-6 flex gap-3 rounded-2xl border border-amber-200 bg-amber-50/80 p-4">
        ...
        {step === 'qr' && selected && amountValid && (
          ...
          >
            ...
            <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm">
              ...
              <motion.div className="mt-5 space-y-3 rounded-xl border border-emerald-100 bg-emerald-50/40 p-4">
                <div>
...
content = content.replace('__DC__', DC).replace('__MC__', MC)
# fix motion.div typo in amber and qr inner - should be div
content = content.replace('<motion.div className="mb-6 flex', '<motion.div className="mb-6 flex')  # keep motion? use div
content = content.replace('<motion.div className="mb-6 flex gap-3', '<div className="mb-6 flex gap-3', 1)
content = content.replace('<motion.div className="mt-5 space-y-3', '<div className="mt-5 space-y-3', 1)
content = content.replace('<motion.div\n        className="flex h-11', '<div\n        className="flex h-11', 1)

Path(__file__).resolve().parents[1].joinpath('src/pages/DepositPage.tsx').write_text(content, encoding='utf-8')
print('written')
